...
class ExemplarHandler(nn.Module, metaclass=abc.ABCMeta):
    ...
    def check_full_memory(self):
        class_budget = self.memory_budget // len(self.online_classes_so_far)
        max_budget = class_budget * len(self.online_classes_so_far)
        current_size = self.get_online_exemplar_size()
        return current_size >= max_budget
    ...
    def check_online_budget(self, n_new):
        return self.get_online_exemplar_size() + n_new < self.memory_budget
    ...
    def drop_instances_out_of_class_budget(self, x, y, n_classes):
        class_budget = self.memory_budget // n_classes
    ...
    def reduce_memory_for_new_classes(self):
        ...
        for m in self.online_classes_so_far:
            n_exemplar_of_m = self.online_exemplar_sets[m][0].shape[0]
            class_budget = self.memory_budget // (len(self.online_classes_so_far) + 1)
            if class_budget == 0:
    ...
    def classify_with_online_exemplars(self, x, allowed_classes=None):
        ...
        # Do the exemplar-means need to be recomputed?
        if self.compute_means:
            ex_means = []  # --> list of 1D-tensors (of size [feature_size]), list is of length [n_classes]
            for k in sorted(self.online_exemplar_sets):
                # Collect all exemplars in P_y into a <tensor> and extract their features
                exemplars = torch.from_numpy(self.online_exemplar_sets[k][0]).to(self._device())
                with torch.no_grad():
                    features = self.feature_extractor(exemplars)
                if self.norm_exemplars:
                    features = F.normalize(features, p=2, dim=1)
                ...
                if self.norm_exemplars:
                    mu_y = F.normalize(mu_y, p=2, dim=1)
                ex_means.append(mu_y.squeeze())  # -> squeeze removes all dimensions of size 1
            # Update model's attributes
            self.exemplar_means = ex_means
            self.compute_means = False
